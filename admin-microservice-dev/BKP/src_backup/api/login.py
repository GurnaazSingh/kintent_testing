...
class Login(Resource,RequestResponse):
    ...
    def get(self):
        ...
        if self.operation==lc_travel_app.LOGIN:
            ...
            try:
                self.update_link_rec = AttrDict(DbFunctions.latest_record(database=lc_travel_app.LC_db,collection_name=lc_travel_app.EVENTS_MASTER,query={"url":self.token}))
                if not user_obj.old_new_pass_checker():
                    user_obj.password_updater()
            except (Exception,InvalidTokenError):
